...
export const runDBTests = async () => {
    ...
    try {
        ...
        if (servers.length === 1 && servers[0].ip_address === '192.168.1.100') console.log('✅ Server Created/Read Success');
        ...
        if (updatedNodes[0].status === 'on' && updatedNodes[0].temperature === 30.0) console.log('✅ Node Update Success');
        ...
        if (alerts.length === 1 && alerts[0].message === 'Overheating') console.log('✅ Alert Created/Read Success');
