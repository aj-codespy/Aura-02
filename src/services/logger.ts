...
class LoggerService {
  async log(level: LogLevel, message: string, meta?: any) {
    try {
      ...
      await db.runAsync(`INSERT INTO logs (level, message, meta, timestamp) VALUES (?, ?, ?, ?);`, [
        level,
        message,
        metaString,
        timestamp,
  ...
  async getLogs(limit = 100, offset = 0) {
    try {
      return await db.getAllAsync(`SELECT * FROM logs ORDER BY timestamp DESC LIMIT ? OFFSET ?;`, [
        limit,
        offset,
      ]);
    } catch (error) {
