...
export default function DeviceListScreen() {
  ...
  const toggleDevice = async (nodeId: number, currentStatus: string) => {
    ...
    // Optimistic update
    setDevices(prev => prev.map(d => (d.id === nodeId ? { ...d, status: newStatus } : d)));
    ...
    try {
      ...
      // Revert on failure
      setDevices(prev =>
        prev.map(d => (d.id === nodeId ? { ...d, status: currentStatus as any } : d))
      );
    }
  };
  ...
  const renderItem = ({ item }: { item: Node }) => (
    <Card style={styles.card}>
      ...
      <Text style={styles.usageText}>Usage Today: {(Math.random() * 10).toFixed(1)} kWh</Text>
    </Card>
  );
